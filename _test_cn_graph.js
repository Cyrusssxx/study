...
(async () => {
  ...
  check('层的统一主色为柔和蓝 #4a7ab8', SRC.includes('--c: #4a7ab8'), true);
  check('深色模式有更亮的主色（#8fb0d9）', SRC.includes('--c: #8fb0d9'), true);
  check('层卡片保留左边框与阴影（不至于太平）', /\.cg-layer \{[^}]*border-left: 3px solid var\(--c\)/.test(SRC.replace(/\n\s*/g, ' ')) && /\.cg-layer \{[^}]*box-shadow/.test(SRC.replace(/\n\s*/g, ' ')), true);
