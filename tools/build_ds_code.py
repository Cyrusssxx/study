...
EXTRA = [
    {
        ...
        "content": ("【2024-41】载人航天工程包含众多子工程，可将子工程间的依赖关系抽象为有向图的拓扑序列问题。"
                    "已知有向图 G 采用邻接矩阵存储：\n"
                    "<pre class=\"code-block\">typedef struct {\n"
        ...
        "answer": ("(1) 基本思想：采用 Kahn（BFS）拓扑排序。先建立各顶点入度表，每次选择入度为 0 的顶点加入拓扑序列，"
                   ...
                   "(2) C 代码：\n"
                   "<pre class=\"code-block\">int uniquely(MGraph G) {                    // 判定是否存在唯一拓扑序列\n"
    ...
    },
    {
        ...
        "answer": ("(1) 基本思想：从后向前一趟扫描。对每个 A[i]，使 A[i]*A[j]（j>=i）最大的 A[j] 必为后缀 A[i..n-1] 中的某个极值："
                   ...
                   "(2) C 代码：\n"
                   "<pre class=\"code-block\">void CalMulMax(int A[], int res[], int n) {\n"
    ...
    },
    {
        ...
        "content": ("【2026-41】二叉搜索树采用二叉链表存储，类型定义如下：\n"
                    "<pre class=\"code-block\">typedef struct BSTNode {\n"
                    "    int data;\n"
                    "    struct BSTNode *left, *right;\n"
                    "} BSTNode;\n"
                    "typedef BSTNode BTNode;</pre>\n"
        ...
        "answer": ("(1) 基本思想：利用 BST 左子树<根<右子树的有序性，与 k 差值最小的结点一定在“沿根向下搜索 k 的路径”上"
                   ...
                   "(2) C 代码：\n"
                   "<pre class=\"code-block\">void SearchX(BTNode *root, int k) {             // 输出与k差值最小的结点\n"
